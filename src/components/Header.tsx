          ...
          <div className="flex justify-between items-center h-20">
           ...
           <Link to="/" className="flex-shrink-0 flex items-center group">
            ...
            <nav className="hidden lg:flex space-x-9">
              {links.map(link => <Link key={link.path} to={link.path} className={`text-xl font-medium transition-colors hover:text-brand-red relative py-2 ${location.pathname === link.path ? 'text-brand-red' : 'text-gray-300'}`}>
                  {link.name}
